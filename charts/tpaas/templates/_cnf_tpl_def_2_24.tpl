...
{{- define "nf.mtcilId" -}}
{{- default "-" $.Values.global.nf.mtcilId | trunc 63 | trimSuffix "-" -}}
{{- end -}}
...
{{- define "nf.nfId" -}}
{{- default "-" $.Values.global.nf.nfId | trunc 63 | trimSuffix "-" -}}
{{- end -}}
...
{{- define "topogw.fqdn" -}}
{{- default "-" $.Values.global.mtcil.topogw_fqdn | trunc 127 | trimSuffix "-" -}}
{{- end -}}
...
{{- default "-" $.Values.app_name_prefix | trunc 63 | trimSuffix "-" -}}
{{- end -}}
...
{{- define "nf.release_name" -}}
{{- $.Release.Name | trunc 63 | trimSuffix "-" -}}
{{ end -}}

{{- define "nf.prefix_2_24" -}}
{{- printf "%s-%s-%s-%s-%s" (include "nf.vendorId" (dict "Values" $.Values) |toString) (include "nf.mtcilId" (dict "Values" $.Values) |toString) (include "nf.nfClass" (dict "Values" $.Values) |toString) (include "nf.nfType" (dict "Values" $.Values) |toString) (include "nf.nfId" (dict "Values" $.Values) |toString) | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{- define "tmaasSpec_2_24" -}}
...
{{- printf "\"mtcilId\": \"%s\"," (.nfVariables.mtcilId | toString) | nindent (add $specOffset 2 | int) -}}
{{- printf "\"nfClass\": \"%s\"," (.nfVariables.nfClass | toString) | nindent (add $specOffset 2 | int) -}}
{{- printf "\"nfType\": \"%s\"," (.nfVariables.nfType | toString) | nindent (add $specOffset 2 | int) -}}
...
{{- define "cnfTplHeader_2_24" -}}
{{- $defaultVariables := $.dot.Values.default -}}
{{- $nfVariables := (dict "nfName" (include "nf.nfType" (dict "Values" $.dot.Values) |toString) "nfType" (include "nf.nfType" (dict "Values" $.dot.Values) |toString) "nfClass" (include "nf.nfClass" (dict "Values" $.dot.Values) |toString) "mtcilId" (include "nf.mtcilId" (dict "Values" $.dot.Values) |toString) "vendorId" (include "nf.vendorId" (dict "Values" $.dot.Values) |toString) "vendorId" (include "nf.vendorId" (dict "Values" $.dot.Values) |toString) "nfId" (include "nf.nfId" (dict "Values" $.dot.Values) |toString) "nfServiceId" (include "nf.component_name" (dict "Chart" $.dot.Chart) |toString) "component" (include "nf.component_name" (dict "Chart" $.dot.Chart) |toString) "nfServiceType" (include "nf.component_name" (dict "Chart" $.dot.Chart) |toString) "svcVersion" (include "nf.version" (dict "Values" $.dot.Values) |toString) "nfPrefix" (include "nf.prefix_2_24" (dict "Values" $.dot.Values) |toString) "svcname" (include "nf.component_name" (dict "Chart" $.dot.Chart) |toString) "component_name" (include "nf.component_name" (dict "Chart" $.dot.Chart) |toString) "topogwFQDN" (include "topogw.fqdn" (dict "Values" $.dot.Values) |toString) "defaultVariables" ($defaultVariables) "root" ($.dot) "create_meta_ns" "true" "release_name" (include "nf.release_name" (dict "Release" $.dot.Release) |toString) ) -}}
{{- if $nfVariables.root.Values.app_name_prefix -}}
{{- $_ := set $nfVariables "nfServiceType" (include "app_name_prefix" (dict "Values" $.dot.Values) |toString) -}}
{{- $_ := set $nfVariables "nfServiceId" (include "app_name_prefix" (dict "Values" $.dot.Values) |toString) -}}
{{- $_ := set $nfVariables "svcname" (include "app_name_prefix" (dict "Values" $.dot.Values) |toString) -}}
{{- end -}}
{{- if and ($nfVariables.root.Values.global) ($nfVariables.root.Values.global.mtcil) ($nfVariables.root.Values.global.mtcil.use_release_ns) -}}
...
{{- define "cnfTplMetadata_2_24" -}}
{{- include "metaSpec_2_24" (merge (dict "specOffset" (add $.setOffset 0 | int) "metaSpec" $.metadata) (dict "nfVariables" $.cnfHdr.nfVariables)) -}}
{{- end -}}
...
{{- define "metaSpec_2_24" -}}
{{- $specOffset := .specOffset -}}
{{- $custom_extensions := .nfVariables.root.Values.global.custom_extensions | default dict -}}
{{- include "eHdr" (dict "key" "metadata" "value" " " "hdrOffset" (add $specOffset 0 | int) ) -}}
{{- if .metaSpec -}}
{{- if .metaSpec.name -}}
{{- include "eHdr" (dict "key" "name" "value" (printf "%s" .metaSpec.name) "hdrOffset" (add $specOffset 2 | int) ) -}}
{{- else if .metaSpec.ext_name -}}
...
{{- include "eHdr" (dict "key" "microSvcName" "value" (.nfVariables.svcname | quote) "hdrOffset" (add $specOffset 4 | int)) -}}
{{- include "eHdr" (dict "key" "component" "value" (.nfVariables.component | quote) "hdrOffset" (add $specOffset 4 | int)) -}}
{{- include "eHdr" (dict "key" "mtcilId" "value" (.nfVariables.mtcilId | quote) "hdrOffset" (add $specOffset 4 | int)) -}}
{{- include "eHdr" (dict "key" "nfType" "value" (.nfVariables.nfType | quote) "hdrOffset" (add $specOffset 4 | int)) -}}
{{- include "eHdr" (dict "key" "nfId" "value" (.nfVariables.nfId | quote) "hdrOffset" (add $specOffset 4 | int)) -}}
{{- include "k8sLabels_2_24" (merge (dict "specOffset" (add $specOffset 4 | int)) (dict "nfVariables" .nfVariables)) -}}
{{- if $custom_extensions.labels -}}
{{- include "customLabels" (merge (dict "specOffset" (add $specOffset 4 | int)) (dict "nfVariables" .nfVariables)) -}}
{{- end -}}
{{- if .metaSpec -}}
...
{{- include "tmaasSpec_2_24" (merge (dict "specOffset" (add $specOffset 4 | int)) (dict "nfVariables" .nfVariables)) -}}
...
{{- if eq .nfVariables.nfType "mtcil" -}}
{{/*- if and .nfVariables.root.Values.global.mtcil.scope.namespaced (or (not .nfVariables.root.Values.global.mtcil.scope.namespaceList) (lt (len .nfVariables.root.Values.global.mtcil.scope.namespaceList) 2)) -*/}}
{{- if and .nfVariables.root.Values.global.mtcil.scope.namespaced (not .nfVariables.root.Values.global.mtcil.scope.namespaceList) -}}
{{- printf "mtcil.com/nf_colocated_ns: \"true\"" | nindent (add $specOffset 4 | int) -}}
{{- end -}}
{{- end -}}
{{- if $custom_extensions.annotations -}}
{{- include "customAnnotations" (merge (dict "specOffset" (add $specOffset 4 | int)) (dict "nfVariables" .nfVariables)) -}}
{{- end -}}
{{- if .metaSpec -}}
...
{{- define "k8sLabels_2_24" -}}
{{- $specOffset := .specOffset -}}
{{- if eq .nfVariables.nfType "mtcil" -}}
{{- include "eHdr" (dict "key" "app.kubernetes.io/name" "value" (.nfVariables.svcname | quote) "hdrOffset" (add $specOffset 0 | int)) -}}
{{- include "eHdr" (dict "key" "app.kubernetes.io/instance" "value" (.nfVariables.release_name | quote) "hdrOffset" (add $specOffset 0 | int)) -}}
{{- end -}}
{{- end -}}

{{- define "customLabels" -}}
{{- $specOffset := .specOffset -}}
{{- range $key, $value := .nfVariables.root.Values.global.custom_extensions.labels }}
{{- include "eHdr" (dict "key" $key "value" ($value | quote) "hdrOffset" (add $specOffset 0 | int)) -}}
{{- end -}}
{{- end -}}

{{- define "customAnnotations" -}}
{{- $specOffset := .specOffset -}}
{{- range $key, $value := .nfVariables.root.Values.global.custom_extensions.annotations }}
{{- include "eHdr" (dict "key" $key "value" ($value | quote) "hdrOffset" (add $specOffset 0 | int)) -}}
{{- end -}}
{{- end -}}
...
{{- $mgmtContents := printf "%s" ($.nfVariables.mgmtpathformat|toString) -}}
{{- range $path, $_ := ($.nfVariables.root.Files.Glob ($mgmtContents|toString)) -}}
{{- if ne (hasSuffix ".tgz" $path) true -}}
...
{{- define "configMapBinaryDataSpec_2_24" -}}
{{- $specOffset := .specOffset -}}
{{- $nfVariables := $.nfVariables -}}
{{- include "eHdr" (dict "key" "binaryData" "value" " " "hdrOffset" (add $specOffset 0 | int) ) -}}
{{- if eq ($.nfVariables.configtype|toString) "mgmt-cfg" -}}
{{- $mgmtContents := printf "%s" ($.nfVariables.mgmtbinpathformat|toString) -}}
{{- range $path, $_ := ($.nfVariables.root.Files.Glob ($mgmtContents|toString)) -}}
{{- $path | base | replace "@" "_rev_" | trimSuffix "\n" | nindent (add $specOffset 2 | int) }}:
{{- ($.nfVariables.root.Files.Get ( $path |toString)) | b64enc | trimSuffix "\n" | nindent (add $specOffset 4 | int) }}
{{- end -}}
{{- end -}}
{{- end -}}
